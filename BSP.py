...
def binary_space_partitioning(rmap, value_to_set, random_walk_iterations, size_limit_x, size_limit_y, max_depth):
    return binary_space_partitioning_random_walk(rmap, value_to_set, random_walk_iterations, size_limit_x, size_limit_y, max_depth, 0)

def binary_space_partitioning_random_walk(rmap, value_to_set, random_walk_iterations, size_limit_x, size_limit_y, max_depth, current_depth):
    ...
    if random.getrandbits(1):
        if size_limit_x + 2 >= x_length - size_limit_x:
            size_limit_reached = True
        if not size_limit_reached:
                split_point = random.randint(size_limit_x, x_length - size_limit_x)
                i = 0
                while i < (y_length):
                    j = 0
                    map_row_1 = []
                    ...
                    while j < (x_length):
                        if j < split_point:
                            map_row_1.append(rmap[i][j])
                        else:
                            map_row_2.append(rmap[i][j])
                        j += 1
                    new_map_1.append(map_row_1)
                    new_map_2.append(map_row_2)
                    i += 1
                if current_depth + 1 > max_depth:
                    new_map_1 = random_walk(new_map_1, random_walk_iterations, False, value_to_set)
                    new_map_2 = random_walk(new_map_2, random_walk_iterations, False, value_to_set)
                else:
                    new_map_1 = binary_space_partitioning_random_walk(new_map_1, value_to_set, random_walk_iterations, size_limit_x, size_limit_y, max_depth, current_depth + 1)
                    new_map_2 = binary_space_partitioning_random_walk(new_map_2, value_to_set, random_walk_iterations, size_limit_x, size_limit_y, max_depth, current_depth + 1)
                returned_map = join_maps_horizontal_bordered(new_map_1, new_map_2, 4)
        else:
            returned_map = random_walk(rmap, random_walk_iterations, False, value_to_set)
                
    else:
        if size_limit_y + 2 >= y_length - size_limit_y:
            size_limit_reached = True
        if not size_limit_reached:
                split_point = random.randint(size_limit_y, y_length - size_limit_y)
                i = 0
                while i < (y_length):
                    if i < split_point:
                        new_map_1.append(rmap[i])
                    else:
                        new_map_2.append(rmap[i])
                    i += 1
                if current_depth + 1 > max_depth:
                    new_map_1 = random_walk(new_map_1, random_walk_iterations, False, value_to_set)
                    new_map_2 = random_walk(new_map_2, random_walk_iterations, False, value_to_set)
                    ...
                    new_map_1 = binary_space_partitioning_random_walk(new_map_1, value_to_set, random_walk_iterations, size_limit_x, size_limit_y, max_depth, current_depth + 1)
                    new_map_2 = binary_space_partitioning_random_walk(new_map_2, value_to_set, random_walk_iterations, size_limit_x, size_limit_y, max_depth, current_depth + 1)
                returned_map = join_maps_vertical_bordered(new_map_1, new_map_2, 4)
        else:
            returned_map = random_walk(rmap, random_walk_iterations, False, value_to_set)
    return returned_map

def join_maps_horizontal_bordered(map_1, map_2, border_value):
    returned_map = []
    i = 0
    while i < len(map_1):
        map_row = []
        j = 0
        while j < len(map_1[i]) - 1:
            map_row.append(map_1[i][j])
            j += 1
        if map_1[i][j] >= 1 and map_2[i][0] >= 1 and random.randint(0, 1):
            map_row.append(border_value)
            map_row.append(border_value)
        else:
            map_row.append(map_1[i][j])
            map_row.append(map_2[i][0])           
        j =  1
        while j < len(map_2[i]):
            map_row.append(map_2[i][j])
            j += 1
        returned_map.append(map_row)
        i += 1
    return returned_map
    
def join_maps_vertical_bordered(map_1, map_2, border_value):
    returned_map = []
    i = 0
    row_length = len(map_1[0])
    while i < len(map_1) - 1:
        map_row = []
        j = 0
        while j < row_length:
    ...
    j = 0
    while j < row_length:
        if map_1[i][j] >= 1 and map_2[0][j] >= 1 and random.randint(0, 1):
            map_row_1.append(border_value)
            map_row_2.append(border_value)
        else:
            map_row_1.append(map_1[i][j])
            map_row_2.append(map_1[0][j])
        j += 1
    returned_map.append(map_row_1)
    returned_map.append(map_row_2)
    i = 1
    while i < len(map_2):
        map_row = []
        j = 0
...
def getDescription():
	return "A map that generates using a Binary Space Partitioning algorithm."
...
def generatePlotTypes():
    ...
    generated_map_to_use = blank_map(iNumPlotsY,iNumPlotsX,0)
    generated_map_to_use = binary_space_partitioning(generated_map_to_use, 1, random_walk_count, 4, 4, 5)
    ...
    generated_map_to_use = cellular_automata(generated_map_to_use, 10, 3)
